---{DISTANCIA 1.19.2}---

A progression and flavoring mod for the Distancia SMP

---FEATURES---

- Ruined Farsteel Item - Does nothing
- Farsteel Bulkhead - Drops 3-9 'Ruined Farsteel' when mined with diamond pickaxe or above
- Farclay Block - Drops 4 'Farclay Ball' when mined with a shovel
- Farclay Ball - Can be crafted into 'Farclay Block,' does nothing
- Nearsteel Ore and Deepslate Variant - Drops 1 'Raw Nearsteel' when mined with diamond pickaxe or above
- Raw Nearsteel - Does nothing

---PLANNED FEATURES---

1. Wall generation
2. Nearsteel
3. Progression system
4. Farsteel refinement

---CHANGELOG---

[Ver 0.0.9-1.19.2 - 12/5/2024]
 Changes:
 - Added 'Deepslate Nearsteel Ore' and associated loot table
 Known Issues:
 - None
 Bug Fixes
 - Somehow accidentally repaired an issue where 'Deepslate Nearsteel Ore' was not behaving properly when mined. I do not
   know how, I just remade the JSONs the same way like 4 times and it started working. I don't fucking know man.
[END]


[Ver 0.0.8-1.19.2 - 12/2/2024]
 Changes:
 - Added 'Raw Nearsteel'
 - Added loot table for 'Nearsteel Ore' now drops 1 'Raw Nearsteel,' requires diamond pickaxe or above
 Known Issues:
 - None
 Bug Fixes
 - Adjusted GUI lighting of all mod elements
[END]

[Ver 0.0.7-1.19.2 - 12/2/2024]
 Changes:
 - Added 'Nearsteel Ore'
 Known Issues:
 - None
 Bug Fixes
 - Adjusted GUI appearance of all mod blocks
[END]

[Ver 0.0.6-1.19.2 - 12/1/2024]
 Changes:
 - Added 'Farclay Ball' item
 - Added loot table for 'Block of Farclay' now drops 4 'Farclay Ball' when mined with a shovel
 - Added shaped crafting recipe for 'Block of Farclay' using 4 farclay balls in a 2 x 2 grid
 Known Issues:
 - None
 Bug Fixes:
 - None
[END]

[Ver 0.0.5-1.19.2 - 11/20/2024]
 Changes:
 - Added block 'Block of Farclay'
 Known Issues:
 - None
 Bug Fixes:
 - None
[END]

[Ver 0.0.4-1.19.2 - 11/20/2024]
 Changes:
 - Added loot table for 'Farsteel Bulkhead'
 Known Issues:
 - None
 Bug Fixes:
 - Fixed texture for 'Farsteel Bulkhead'
[END]

[Ver 0.0.3-1.19.2 - 11/19/2024]
 Changes:
 - Added 'Farsteel Bulkhead'
 Known Issues:
 - Texture is failing to load for 'Farsteel Bulkhead'
 Bug Fixes:
 - None
[END]

[Ver 0.0.2-1.19.2 - 10/30/2024]
 Changes:
 - Renamed mod to 'Distancia'
 - Refactored mod id
 - Added 'Ruined Farsteel'
 - Added the creative mode tab for the mod
 Known Issues:
 - None
 Bug Fixes:
 - None
[END]

[Ver 0.0.1-1.19.2 - 10/28/2024]
 Changes:
 - Created mod
 - Renamed mod to 'Farcraft'
 Known Issues:
 - None
 Bug Fixes:
 - None
[END]











